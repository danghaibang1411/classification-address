...
class AddressCorrection:
            ...
            for district, distance_district in district_candidates:
                ...
                if district_index > 0:
                    ...
                    def check_prefix():
                        ...
                        if district_index < 2:
                            return new_district_index, prefix_district, distance
                        d = self.string_distance.distance(' '.join(tokens[district_index - 2: district_index]),
                                                          'thành phố')
                        if d <= 20:
                            prefix_district = 'thành phố'
                ...
                if index_province > 0:
                    if tokens[index_province - 1] == 'tp':
                        ...
                        if result_distance_candidate < result_distance:
                            result_distance = result_distance_candidate
                            result = result_candidate
                    elif index_province > 1 and self.string_distance.distance(
                            ' '.join(tokens[index_province - 2:index_province]), 'thành phố') < 20:
                        if index_province <= 1:
    ...
    def address_correction(self, address, correct_th=60):
        '''
